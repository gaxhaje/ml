...
def key_stats(gather=['Total Debt/Equity',
  ...
  sp500_df = pd.DataFrame.from_csv('sp500_index.csv')
  stock_df = pd.DataFrame.from_csv('sp500_ticker_prices.csv')
      ...
      for file in each_file:
        ...
        try:
          ...
          try:
            ...
            sp500_price = round(float(row['Adj Close']))
          except:
            sp500_date = datetime.fromtimestamp(unix_time-259300).strftime('%Y-%m-%d')
            row = sp500_df[(sp500_df.index == sp500_date)]
            sp500_price = round(float(row['Adj Close']))
          ...
          try:
            ...
            sp500_1y_price = -99999999 if row.empty else round(float(row['Adj Close']))
          except:
            try:
              ...
              print('sp500 1 year later issuer:', str(e))
          ...
          try:
            ...
            if not row.empty:
              stock_1y_price = round(float(row),2)
          except:
            try:
              ...
              print('stock price 1y later:', str(e))
